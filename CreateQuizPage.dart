...
class _CreateQuizPageState extends State<CreateQuizPage> {
  ...
  void _saveQuiz() async {
      if (!_formKey.currentState!.validate()) {
        setState(() => _feedback = 'Erro: Certifique-se de que todas as informações estão corretas.');
        return;
      }
      if (_startDate == null || _endDate == null) {
        setState(() => _feedback = 'Erro: As datas de início e término precisam ser selecionadas.');
        return;
      }
      if (_endDate!.isBefore(_startDate!)) {
        setState(() => _feedback = 'Erro: A data de término deve ser posterior à data de início.');
        return;
      }
      if (_questions.isEmpty) {
        setState(() => _feedback = 'Erro: Pelo menos uma questão é necessária para o quiz.');
        return;
      }
      for (var question in _questions) {
        if (question.questionText.isEmpty || question.options.any((option) => option.isEmpty)) {
          setState(() => _feedback = 'Erro: Cada questão precisa ter um texto e opções não vazias.');
          return;
        }
        if (question.correctOptionIndex < 0 || question.correctOptionIndex >= question.options.length) {
          setState(() => _feedback = 'Erro: O índice da opção correta está fora do intervalo.');
          return;
        }
      }
